...
@section('content')
    ...
    <form class="content" method="POST" enctype="multipart/form-data">
        ...
        <div class="row mb-3">
            ...
            <div class="col-9">
                <input type="file" id="imgInp" name="image" value="{{ asset('upload/image/'.$post->image) }}" accept="image/*" onchange="previewFile(this);">
                <div class="col-md-12 mb-2">
                    <img class="img-thumbnail" id="previewImg" src="{{ asset('upload/image/'.$post->image) }} "
                        alt="your image" />
                </div>
                @error('image')
                    <span style="color: red">{{ $message }}</span>
        ...
        <div class="row mb-3">
            ...
            <div class="col-9">
                <textarea class="form-control ckeditor" id="editor" name="content" cols="80" rows="10" >{{ $post->content }}</textarea>
                @error('content')
                    <span style="color: red">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div class="col-9">
                @foreach ($categoryList as $item)
                    <div class="form-check text-uppercase">
                        ...
                        {{ in_array($item->id,$post->category->pluck('id')->toArray()) ? 'checked': '' }}
                        >
